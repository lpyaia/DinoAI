...
def on_press(key):
    ...
    if k in ['up', 'down', 'n']:  # keys of interest
        # self.keys.append(k)  # store it in global-like variable

        if(k == 'n'): vision.resetCurrentSpeed()
        else: print(f"Key pressed: {k} - distX: {distX} - distY: {distY} - current speed: {currentSpeed}")
...
while(True):
    distX, distY, currentSpeed = vision.getClosestEnemy()
